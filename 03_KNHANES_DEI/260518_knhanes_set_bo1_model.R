...
#   <dbl> <int>         |   <dbl> <int>
#1      0  5348 마른편  |1      0  1279 마른편
#2      1  5438 뚱뚱한편|2      1  9507 뚱뚱한편
...
df_004 <- df_003 %>% select(sex, edu_binary, marri_1, town_t, EC1_1, ainc_binary, 
                            ...
                            DI1_dg, DI2_dg, DE1_dg, model1, model2)
...
df_fit_001 <- df_004 %>% mutate(
  ...
  model1 = factor(model1, levels = c(0, 1), labels = c('마른편', '뚱뚱한편')),
  model2 = factor(model2, levels = c(0, 1), labels = c('마른편', '뚱뚱한편')),
) %>% rename(
  성별 = sex,
  교육수준 = edu_binary,
  결혼여부 = marri_1,
