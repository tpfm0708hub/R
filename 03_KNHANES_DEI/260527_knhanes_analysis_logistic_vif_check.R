...
summary_vif <- function(df_01, col_01){
  ...
  sum_cov_01 <- formula(paste(col_01 ,'~', paste(col_list_01, collapse = ' + ')))
  sum_cov_02 <- glm(sum_cov_01, data = df_02, family = quasibinomial(link = 'logit'), weights = wt_itvex)
...
# model1
# 당뇨경험집단
summary_vif(df_diabetes_001, 'model1')
#                    성별                 연령구분                 교육수준                 결혼여부 
#                1.440641                 1.241601                 1.140437                 1.274867 
#                거주지역             경제활동상태             중위가구소득                   흡연자 
#                1.043182                 1.220933                 1.160160                 1.266078 
#          고위험음주여부           유산소실천여부             스트레스인지           범불안장애위험 
#                1.169011                 1.041802                 1.253622                 1.235275 
#                비만여부       고혈압의사진단여부 이상지질혈증의사진단여부 
#                1.103210                 1.103965                 1.124724 
# 일반집단
summary_vif(df_non_diabetes_001, 'model1')
#                    성별                 연령구분                 교육수준                 결혼여부 
#                1.296228                 1.567860                 1.114836                 1.455693 
#                거주지역             경제활동상태             중위가구소득                   흡연자 
#                1.031689                 1.081355                 1.060943                 1.234408 
#          고위험음주여부           유산소실천여부             스트레스인지           범불안장애위험 
#                1.107394                 1.043743                 1.218010                 1.199094 
#                비만여부       고혈압의사진단여부 이상지질혈증의사진단여부 
#                1.093341                 1.290792                 1.253003 
