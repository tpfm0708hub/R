...
# BE3_71(고강도 신체활동 여부: 일) '모름, 무응답'(9) 제거
...
# BE3_75(고강도 신체활동 여부: 여가) '모름, 무응답'(9) 제거
df_003 <- df_003 %>% filter(BE3_71 != 9)

# BE3_82(고강도 신체활동 일수: 여가) 빈도 확인
#df_003 %>% count(BE3_76)

# BE3_76(고강도 신체활동 일수: 여가) '모름, 무응답'(9) 제거
...
# BE3_92(신체활동 일수: 장소이동) '모름, 무응답'(9) 제거
df_003 <- df_003 %>% filter(BE3_92 != 9)

# pa_aerobic(유산소신체활동여부) 변수 생성(0: 비해당, 1: 해당)
# 지표 정의(다음 중 하나 해당)
# 일주일에 중강도 신체활동: 2시간30분 이상
# 고강도 신체활동: 1시간 15분 이상 
...
#  model1  vars     n  mean    sd median trimmed   mad   min   max range   skew kurtosis    se
#   <dbl> <dbl> <dbl> <dbl> <dbl>  <dbl>   <dbl> <dbl> <dbl> <dbl> <dbl>  <dbl>    <dbl> <dbl>
#1      0     1  5348  44.6  13.6     46    45.1  16.3    19    64    45 -0.272   -1.19  0.185
#2      1     1  5438  45.3  12.4     47    45.9  14.8    19    64    45 -0.315   -0.952 0.168
df_003 %>% group_by(model2) %>% summarise(describe(age))
#  model2  vars     n  mean    sd median trimmed   mad   min   max range   skew kurtosis    se
#   <dbl> <dbl> <dbl> <dbl> <dbl>  <dbl>   <dbl> <dbl> <dbl> <dbl> <dbl>  <dbl>    <dbl> <dbl>
#1      0     1  1279  43.2  14.5     44    43.5  20.8    19    64    45 -0.118    -1.37 0.404
#2      1     1  9507  45.2  12.8     47    45.8  14.8    19    64    45 -0.318    -1.02 0.131
...
df_004 <- df_003 %>% select(sex, edu_binary, marri_1, town_t, EC1_1, ainc_binary, 
                            smoker, dr_high, BP1_binary, mh_GAD_binary, obesity_binary, 
                            DI1_dg, DI2_dg, age_binary, DE1_dg, model1, model2)
