...
# 현재 나이와 당뇨병 진단시기 간 차이 확인
nrow(df_003)
# 16647
df_003 %>% pull(DE1_ag) %>% describe()
#   vars     n   mean     sd median trimmed mad min max range  skew kurtosis   se
#X1    1 16647 784.34 274.97    888  862.81   0  17 999   982 -2.27     3.17 2.13

# 당뇨병 진단시기 변수 결측값 확인
# 나이 기입 변수에서, '80세이상' 응답보다 큰 범주 빈도 확인
df_003 %>% filter(DE1_ag > 80) %>% select(DE1_ag) %>% count(DE1_ag)
#  DE1_ag     n 내용
#1    888 14562 비해당(소아, 청소년, 의사진단 받지 않음)
#2    999    10 모름, 무응답

# 당뇨병 진단시기 결측 처리 및 당뇨진단 경과기간(DE1_gap) 생성
# 888(비해당): 결측처리
# 999(무응답): 분석에서 제외
df_004 <- df_003 %>% filter(DE1_ag != 999) %>% 
  mutate(DE1_ag = case_when(DE1_ag == 888 ~ NA,
                            TRUE ~ DE1_ag),
         DE1_gap = age - DE1_ag
         )

# DE1_gap 분포 확인
df_004 %>% pull(DE1_gap) %>% describe(.)
#   vars    n mean   sd median trimmed  mad min max range skew kurtosis   se
#X1    1 2075 9.96 8.81      7    8.76 7.41   0  56    56 1.22     1.37 0.19