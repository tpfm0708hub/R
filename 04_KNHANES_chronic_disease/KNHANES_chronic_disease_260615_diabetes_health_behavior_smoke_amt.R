...
# 하루평균 일반담배(궐련) 흡연량(BS3_2) 분포 확인
df_005 %>% pull(BS3_2) %>% describe()

# 궐련형 전자담배 흡연자 하루평균 흡연량(BS12_47_1) 분포 확인
df_005 %>% pull(BS12_47_1) %>% describe()
...
# 생성변수 현재 흡연량 분포 확인
df_006 %>% pull(smoke_amt) %>% describe(.)